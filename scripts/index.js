...
d3.json("data/connections.json", function (error, data) {
    ...
    var node = svg.selectAll(".node")
        ...
        .on('mouseover', function (d) {
            ...
            if (!lockedLink) {
                ...
                var bacon = 0;
                for (var i = 0; i < 85; i++) {
                    if (i == graph.nodes[d.id].connectedArtists[bacon]) {
                        bacon++;
                    }
                    else if (i == d.id) {
                        HighlightCenteredNode(i);
                    }
                    else {
                        FadeOutNode(i);
                    }
                ...
                for (var i = 0; i < edgeNum; i++) {
                    if (i == graph.nodes[d.id].connections[bacon]) {
                        bacon++;
                        HighlightEdge(i);
                    }
                    else {
                ...
                document.getElementById('ArtistLink').innerHTML = graph.nodes[d.id].connections.length;
    ...
    for (var i = 0; i < 85; i++) {
        d3.select('#node' + i)
            .append('circle').attr('r', '6.2px')
            .attr('id', "circle" + i).attr("fill", colorSet[graph.nodes[i].group])
...
function HighlightCenteredNode(id) {
    d3.select("#circle" + id).classed('node-centered-', false).classed('node-centered', true)
}

function De_HighlightCenteredNode(id) {
    d3.select("#circle" + id).classed('node-centered', false).classed('node-centered-', true)
}

function FadeOutNode(id) {
    d3.select("#circle" + id).attr('opacity', 0.2);
}
function FadeInNode(id) {
